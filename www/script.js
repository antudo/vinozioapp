...
function dom()
{
		...
		else
		{
			...
			$('.risposte').stop().animate({"margin-left":"-100%"},500,function(){
				...
				$('.risposte').show();
				$('.risposte').find('table').stop().animate({width: '100%'},800,function(){$('.risposta span').fadeIn(500);});
...
function init_catalogo(query,tour)
{
	...
	leggi_db(dati,function(res){
		...
		for(i=0;i<rows.length;i++)
		{
			...
			if(tipotag.indexOf('anco')!=-1)
				tipotag=lingua[13];
			else
			if(tipotag.indexOf('umante')!=-1)
				tipotag=lingua[14];
			else
			if(tipotag.indexOf('osat')!=-1)
				tipotag=lingua[15];
				...
				if(pack=='111')
				{
					...
					if($('.filter').attr('data-fnaz')=='0')
					{
						...
						$('.filColori div').not('.legend').each(function(){
							...
							if(!$('.red').hasClass('selezione') &&
								!$('.white').hasClass('selezione') &&
								!$('.spum').hasClass('selezione') &&
								!$('.rosa').hasClass('selezione')  )
